...
float getLeftSR() {
  ...
  // float rawDist = 12.08 * pow(volt, -1.058); // simplified curve from
  // datasheet distSR2 = kfSR_L.updateEstimate(rawDist);
  float adcRaw = analogRead(pinIR_Short2);
  if (adcRaw == 0) adcRaw = 1;
  float lastLeftSR =
      pow((adcRaw / 1562610.0), (1.0 / -1.98778));  // sevans calibration
