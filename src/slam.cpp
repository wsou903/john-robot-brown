...
void budget_slam()
{
    ...
    auto addCoordinateEstimate = [&](float distance, float mounted_angle)
    {
        ...
        {
            if (s > 0) sum_y += distance * abs(s);
            else       sum_y += TABLE_WIDTH - (distance * abs(s));
            y_count++;
        }
    };
    ...
    if (x_count > 0) robotX = sum_x / x_count;
    if (y_count > 0) robotY = sum_y / y_count;
